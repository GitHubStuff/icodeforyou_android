# 24.1.12 — Phase 1: App Dependency — `lifecycle-runtime-compose`
...
- **Added** `implementation(libs.androidx.lifecycle.runtime.compose)`
  directly below `lifecycle-runtime-ktx`, keeping the lifecycle family
  grouped. References the catalog entry declared in 24.1.11.
- Nothing else touched — plugins, android block, Room schema config,
  BuildConfig key, and every other dependency are byte-identical to the
...
Gradle sync resolves `androidx.lifecycle:lifecycle-runtime-compose:2.11.0`;
build compiles clean. No behavior change — nothing imports
`collectAsStateWithLifecycle` until 24.1.13 (`MainActivity`).
