# 24.2.7 — Phase 2: App Dependency — `sqlite-bundled`

**Project:** CatsCarsCoins (`com.icodeforyou.catscarscoins`)
**Spec section:** 24.2 — Coins (Phase 2)
**File delivered:** `app/build.gradle.kts` (complete file)
**Date:** 2026-07-05
...
- **Added** `implementation(libs.sqlite.bundled)` directly under the Room 3
  pair (`room3-runtime` / `ksp(room3-compiler)`), keeping the persistence
  family grouped. References the catalog entry from 24.2.6.
- Nothing else touched — base is the 24.1.12 file; every other line is
  byte-identical.

## APK size note

The bundled driver ships compiled SQLite natives per ABI — a few MB of APK
growth versus the framework driver. Accepted deliberately: deterministic
SQLite across devices is worth megabytes in a reference blueprint, and the
Cats FTS work (Phase 3) stops depending on the device's platform SQLite
age.
...
Gradle resolves `androidx.sqlite:sqlite-bundled:2.6.1`; build compiles
clean. Nothing imports the driver until the Koin database module (next
step).
