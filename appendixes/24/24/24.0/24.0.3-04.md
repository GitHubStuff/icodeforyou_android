## 24.0.3-04 — `app/build.gradle.kts` + Sync

Replace the entire contents of `app/build.gradle.kts` (**Android view:** Gradle Scripts → `build.gradle.kts (Module :app)`. **Project view:** `CatsCarsCoins/app/build.gradle.kts`) with the file below.

Then run the single sync for 24.0.3-02 → -04: **File → Sync Project with Gradle Files.** First sync downloads ~30 new artifacts — slower than normal, expected. Green sync = all catalog pins proven; the "alias not used" warnings from 24.0.2 drop to zero.

**Why:** every change from the wizard file is marked inline — `── ADDED` (new) or `── UPDATED` (old value shown); unmarked lines are wizard output, untouched. The `kotlin-serialization` and `ksp` aliases apply with no version (resolved from the root buildscript classpath, 24.0.3-02). Do **not** add `alias(libs.plugins.kotlin.android)` — AGP 9's built-in Kotlin forbids it, and the IDE's "Fix with AI" has been observed injecting it.
