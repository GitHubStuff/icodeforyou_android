# 24.3.32 — Phase 3: Register `CatDetailViewModel` (Parameterized)
...
1. **Added** a parameterized `viewModel { parameters -> ... }` factory:
   `catId = parameters.get()` (fed by `parametersOf(catId)` from
   `CatDetailScreen`), repository from the graph — the app's first
   runtime-parameter ViewModel.
2. **Imports:** `viewModel` DSL (the factory form; `viewModelOf` stays for
   the no-arg `CatsViewModel`), `CatDetailViewModel`.
3. Everything else unchanged from 24.3.20.

## Verification gate

1. `./gradlew assembleDebug`
2. Expect: compiles clean; the detail screen can now resolve its ViewModel
   once the nav entry + row tap land (24.3.33–34).
...
// CatsCarsCoins — spec 24.3.32. Complete file.
// Change from 24.3.20: CatDetailViewModel registered with a catId
// parameter (parametersOf at the call site).
...
import com.icodeforyou.catscarscoins.cats.ui.CatDetailViewModel
...
val catsModule = module {
    ...
    // Parameterized: the catId flows in via parametersOf at the call site
    // (CatDetailScreen). The repository resolves from the graph.
