# 24.3.9 — Phase 3: TDD Red — `RoomCatsRepository` Instrumented Tests

**Project:** CatsCarsCoins (`com.icodeforyou.catscarscoins`)
**Spec section:** 24.3 — Cats (Phase 3); spec §24.6 test matrix
**File delivered:** `app/src/androidTest/java/com/icodeforyou/catscarscoins/cats/data/RoomCatsRepositoryTest.kt` (complete file, new)
**Date:** 2026-07-05
...
RoomCatsRepository(
    catDao: CatDao,
    remoteSource: CatsRemoteSource,
) : CatsRepository
```

Real Room + FTS4 over the bundled driver, in-memory; the remote port
hand-faked (settable page). FTS behavior is the SQL worth testing on real
SQLite — the same reasoning as 24.2.10.

## Behaviors pinned (ten assertions across nine tests)

- **Contract mechanics:** blank → all, breed-alphabetical; no match →
  empty; refresh twice with the same page → 3 rows, not 6; same id with
  changed data → updated in place (upsert, not insert-or-ignore);
  `clearAll` empties both direct listing and FTS results.
- **The MATCH translation policy, by behavior:** lowercase hits;
  **"agi" finds Agile** — the implementation must append a prefix `*` per
  token; **"alert agile" requires both tokens**; origin and description
  are reachable (all four indexed fields exercised across the suite).
...
**Passes when compilation FAILS** on unresolved `RoomCatsRepository`.
Green (24.3.10) gates on `:app:connectedDebugAndroidTest` — which also
still owes the 24.3.6 migration test its first on-device verdict.

---

## Complete file: `RoomCatsRepositoryTest.kt`
