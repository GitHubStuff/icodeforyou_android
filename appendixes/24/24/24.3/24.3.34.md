# 24.3.34 — Phase 3: Wire Cat Detail Navigation
...
Built on YOUR 24.3.25 baseline — the hand-authored splash/system-bar
orchestration (`DisposableEffect` themed bars, background-behind-notch,
`LocalActivity`). Only the detail navigation is added; nothing in your
splash work is touched.

## Changes

1. **`entry<CatsKey>`** now passes
   `onCatSelected = { catId -> backStack.add(CatDetailKey(catId)) }` — the
   row tap pushes the detail key ON TOP of Cats, so system back pops
   naturally to the list.
2. **New `entry<CatDetailKey> { key -> CatDetailScreen(catId = key.catId) }`**
   — the parameterized screen resolves its ViewModel via
   `parametersOf(catId)`.
3. **Imports:** `CatDetailKey`, `CatDetailScreen`. Rail, insets, splash,
   all other entries — unchanged.

## Detail is not a rail destination

`CatDetailKey` is pushed, never a `TOP_LEVEL_DESTINATIONS` item — the rail
still shows Main / Cats / Coins / Settings. Detail lives above Cats in the
stack; the rail's Cats item stays selected.

## Verification gate — cat detail end to end

1. `./gradlew assembleDebug`
2. Cats → Refresh → tap any breed row → detail opens: full-width image,
   origin, life span, temperament, description.
3. System back → returns to the list, query and scroll intact.
4. A background refresh while detail is open updates the shown cat live
   (the observe-don't-snapshot design).
5. Rail Cats item stays selected throughout; other rail taps still work.

Passing this completes the cat detail feature — one of the two remaining
Phase 3 spec items. The last is the generic `SwipeableRefreshableList<T>`
extraction.
